...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $instrumentoMusical = isset($_POST['instrumentoMusical']) ? trim($_POST['instrumentoMusical']) : '';
    ...
    $fotoBeneficiario = isset($_FILES['fotoBeneficiario']['tmp_name']) ? file_get_contents($_FILES['fotoBeneficiario']['tmp_name']) : null;

    // Verificar que todos los campos requeridos estén completos
    if (!empty($primerNombre) && !empty($primerApellido) && !empty($tipoDocumento) && !empty($numeroDocumento) && !empty($fechaNacimiento) && !empty($edad) && !empty($lugarNacimiento) && !empty($genero) && !empty($tipoSanguineo) && !empty($direccion) && !empty($estrato) && !empty($correoElectronico) && !empty($telefono) && !empty($centroMusical) && !empty($instrumentoMusical) && !empty($semestre) && !empty($ano) && !empty($fecha) && $acepto_terminos && !empty($fotoBeneficiario)) {
        // Preparar la consulta de inserción para la tabla Beneficiario
        $consultaBeneficiario = "INSERT INTO beneficiario (primerNombre, segundoNombre, primerApellido, segundoApellido, tipoDocumento, numeroDocumento, fechaNacimiento, edad, lugarNacimiento, genero, tipoSanguineo, direccion, estrato, correoElectronico, telefono, centroMusical, instrumentoMusical, semestre, ano, fecha, fotoBeneficiario) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        if ($stmtBeneficiario === false) {
            ...
            $stmtBeneficiario->bind_param("sssssssisssisssssbsss", $primerNombre, $segundoNombre, $primerApellido, $segundoApellido, $tipoDocumento, $numeroDocumento, $fechaNacimiento, $edad, $lugarNacimiento, $genero, $tipoSanguineo, $direccion, $estrato, $correoElectronico, $telefono, $centroMusical, $instrumentoMusical, $semestre, $ano, $fecha, $fotoBeneficiario);
